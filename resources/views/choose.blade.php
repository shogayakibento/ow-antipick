...
<div style="padding: 0 4px;">
    ...
    <div style="text-align: center; margin-bottom: 8px;">
        ...
        <p style="font-size: 0.78rem; color: #3a4560;">
            相性スコアの高い順にランキング形式で表示 &nbsp;|&nbsp;
            <span style="color: #4a9eff;"><i class="bi bi-info-circle-fill"></i> カードをタップで理由を確認</span>
        </p>
    </div>
        ...
        @foreach($groupedCharacters as $rank => $characters)
            <div class="rank-header rank{{ $rank }}">
                ...
                <span style="font-size: 0.8rem; font-weight: 400; margin-left: 8px; opacity: 0.7;">
                    スコア: {{ $scores[$characters->first()->id] ?? 0 }}
                </span>
            </div>
                    ...
                    >
                        ...
                        @if(isset($reasons[$character->id]) && count($reasons[$character->id]) > 0)
                            <div style="font-size: 0.6rem; color: #4a9eff; margin-top: 2px; letter-spacing: 0.03em;">
                                <i class="bi bi-info-circle-fill"></i> 理由を見る
                            </div>
                        @endif
                    </div>
                                    ...
                                    <div style="display: flex; align-items: center; gap: 12px;">
                                        ...
                                        <div>
                                            ...
                                            <div style="font-size: 0.75rem; color: #8892a4;">
                                                スコア: <span style="color: #4a9eff; font-weight: 700;">{{ $scores[$character->id] ?? 0 }}</span>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="modal-body" style="padding: 16px 18px; max-height: 55vh; overflow-y: auto;">
                                            ...
                                            ">
                                                <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 6px;">
                                                    ...
                                                    @if($reasonData['score'] >= 2)
                                                        <span style="font-size: 0.65rem; background: rgba(239,68,68,0.2); color: #ef4444; padding: 1px 6px; border-radius: 10px; border: 1px solid rgba(239,68,68,0.3);">強カウンター</span>
                                                    @else
                                                        <span style="font-size: 0.65rem; background: rgba(245,158,11,0.2); color: #f59e0b; padding: 1px 6px; border-radius: 10px; border: 1px solid rgba(245,158,11,0.3);">有利</span>
                                                    @endif
                                                </div>
                                    ...
                                    @else
                                        <p style="color: #5a6278; font-size: 0.85rem; text-align: center; padding: 20px 0;">
                                            <i class="bi bi-info-circle" style="margin-right: 6px;"></i>詳細情報がありません
                                        </p>
                                    @endif
                                </div>
                                <div class="modal-footer" style="border-color: #2a3348; padding: 10px 18px; background: #161b27;">
                                    <a href="{{ route('hero.show', $character->slug) }}" style="font-size: 0.8rem; color: #4a9eff; text-decoration: none; font-weight: 600;" onmouseover="this.style.textDecoration='underline'" onmouseout="this.style.textDecoration='none'">
                                        <i class="bi bi-box-arrow-up-right" style="margin-right: 4px;"></i>{{ $character->name }} の詳細ページ
                                    </a>
                                </div>
                            </div>
                        </div>
