...
@section('title', $hero->name_en . ' Counters | OW Anti-Pick Checker')
@section('description', 'Heroes that counter ' . $hero->name_en . ' in Overwatch. Ranked by counter score with detailed reasons.')
@section('content')
<div style="padding: 0 4px;">
    ...
    <div style="
        ...
        gap: 16px;
    ">
        <img src="{{ $hero->image_url }}" alt="{{ $hero->name_en }}"
        ...
        <div>
            <p style="font-size: 0.75rem; color: #ef4444; font-weight: 700; margin-bottom: 4px; letter-spacing: 0.05em; text-transform: uppercase;">
                <i class="bi bi-person-fill" style="margin-right: 4px;"></i>Your Hero
            </p>
            <h1 style="font-size: 1.3rem; font-weight: 800; color: #e8eaf6; margin: 0 0 2px;">{{ $hero->name_en }}</h1>
        </div>
        <div class="ms-auto">
            <a href="{{ route('en.hero.show', $hero->slug) }}"
                ...
                <i class="bi bi-info-circle" style="margin-right: 3px;"></i>Details
            </a>
        </div>
    </div>
    ...
    <div style="text-align: center; margin-bottom: 16px;">
        <h2 style="font-size: 1.1rem; font-weight: 800; color: #e8eaf6; margin-bottom: 4px;">
            <i class="bi bi-shield-exclamation" style="color: #ef4444; margin-right: 8px;"></i>Heroes that counter {{ $hero->name_en }}
        </h2>
        <p style="font-size: 0.78rem; color: #3a4560;">
            Sorted by counter score &nbsp;|&nbsp; {{ $weaknesses->count() }} heroes total
        </p>
    </div>

    @if($weaknesses->isEmpty())
        <div style="text-align: center; padding: 40px 20px; background: #1e2535; border-radius: 12px; border: 1px solid #2a3348;">
            <p style="color: #5a6278; font-size: 0.9rem;">No counter data available</p>
        </div>
    @else
    ...
    <div style="display: flex; gap: 10px; flex-wrap: wrap;">
        ...
        ">
            <i class="bi bi-arrow-repeat"></i>Pick another hero
        </a>
